...
bool RH_E32::init(uint8_t mode)
{
  ...
  myparameters.addh = 0x00;
  myparameters.addl = 0x00;
  myparameters.sped = SPED;
  myparameters.chan = 0x0F;
  myparameters.option = OPTION;

  writeParameters(myparameters, true);

  if (mode==1){           //transmitter
    setMode(RHModeTx);
  }
...
void RH_E32::validateRxBuf()
{
  ...
  bool RH_E32::available()
  {
    ...
    bool RH_E32::send(const char * data, uint8_t len)
    {
    ...
    {
      if (frequency < 900 || frequency > 931)
      return false;

      Parameters params;
      if (!readParameters(params))
      return false;
      params.chan = frequency - 900;
      return writeParameters(params);
